...
CREATE Table mallsandsupermarketweekend
(
   --ID int identity,
...
SELECT * from mallsandsupermarketweekend;
...
Begin 
	...
	Insert Into mallsandsupermarketweekend values (@RandomMallsPlaceID, @RandomDateTimeStamp)
...
Select z.MallsPlaceId,
		z.DateTimeStamp,
		z.OccupancyPercent into mallsandsupermarkettbweekend from 
		(select *, 
		OccupancyPercent=case
						...
						then abs(Round((60 - 50) * Rand(checksum(newid())) + 50, 0))
						...
						End 
		from mallsandsupermarketweekend) 
		as z ;

		--drop table  mallsandsupermarkettb;
		
Select * from mallsandsupermarkettbweekend;
